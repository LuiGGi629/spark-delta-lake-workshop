...
# MAGIC %sql
# MAGIC CREATE OR REPLACE TABLE AS VALUES
# MAGIC   ("05:49:56.604899", "10.0.0.2.54880", "10.0.0.3.5001",  2) labels(column0, column1, column2, label)

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT * FROM labels
...
# MAGIC SELECT
# MAGIC   w,
# MAGIC   ARRAY_SORT(COLLECT_SET(id)) as ids
# MAGIC FROM (
# MAGIC   SELECT 
# MAGIC   *,
# MAGIC   -- Is EXPLODE a LATERAL VIEW?
# MAGIC   EXPLODE(SPLIT(words,",")) AS w
# MAGIC   FROM (VALUES (1,"one,two,three","one"),
# MAGIC           (2,"four,one,five","six"),
# MAGIC           (3,"seven,nine,one,two","eight"),
# MAGIC           (4,"two,three,five","five"),
# MAGIC           (5,"six,five,one","seven")) AS t(id,words,word) 
# MAGIC )
# MAGIC WHERE w IN ('five','one','six','seven')
# MAGIC GROUP BY w
# MAGIC ORDER BY w
...
# MAGIC %sql
# MAGIC -- Optionally, register the dataset as a table (for SQL queries)
# MAGIC CREATE OR REPLACE TABLE words AS
# MAGIC VALUES
# MAGIC   (1,"one,two,three","one"),
# MAGIC   (2,"four,one,five","six"),
# MAGIC   (3,"seven,nine,one,two","eight"),
...
# MAGIC from pyspark.sql import functions as F
# MAGIC 
# MAGIC cities_with_pop_long = cities.withColumn("pop", F.translate("population", " ", "").cast("long"))
# MAGIC biggest_cities_per_country = cities_with_pop_long.groupBy("country").agg(F.max("pop").alias("max_population"))
# MAGIC solution = (
# MAGIC     biggest_cities_per_country.join(cities_with_pop_long, ["country"])
...
population_no_spaces = cities.withColumn("population_no_spaces", F.translate("population", " ", "").cast("long"))
by_country = Window.partitionBy("country").orderBy(F.desc("population_no_spaces"))
solution = (
    population_no_spaces.withColumn("rank", F.rank().over(by_country))
    .where(F.col("rank") == 1)
...
# MAGIC %sql
# MAGIC -- Kuba's Solution
# MAGIC -- FIXME type of the population column
# MAGIC SELECT 
# MAGIC     orig.name,
# MAGIC     pars.country,
# MAGIC     pars.population
# MAGIC FROM(
# MAGIC     SELECT 
# MAGIC         country,
# MAGIC         FIRST(name) AS name,
# MAGIC         MAX(population) AS population
# MAGIC     FROM (
# MAGIC         SELECT * 
# MAGIC         FROM cities
...
# MAGIC ) AS pars
# MAGIC LEFT JOIN cities AS orig
# MAGIC ON pars.country = orig.country AND pars.name = orig.name

# COMMAND ----------

# MAGIC %sql
# MAGIC -- Yegor's solution
# MAGIC -- Mind it uses window aggregation
# MAGIC SELECT 
# MAGIC         name, 
# MAGIC         country, 
# MAGIC         population
...
# MAGIC         SELECT 
# MAGIC             *,
# MAGIC             MAX(CAST(REGEXP_REPLACE(population," ","") AS LONG)) OVER (PARTITION BY country) AS m
# MAGIC         FROM (VALUES ("Warsaw","Poland","1 764 615"),
# MAGIC                     ("Cracow","Poland","769 498"),
# MAGIC                     ("Paris","France","2 206 488"),
# MAGIC                     ("Villeneuve-Loubet","France","15 020"),
# MAGIC                     ("Pittsburgh PA","United States","302 407"),
# MAGIC  ("Chicago IL","United States","2 716 000"),
# MAGIC                     ("Milwaukee WI","United States","595 351"),
# MAGIC                     ("Vilnius","Lithuania","580 020"),
# MAGIC                     ("Stockholm","Sweden","972 647"),
# MAGIC                     ("Goteborg","Sweden","580 020")) AS t(name,country,population))
# MAGIC     WHERE CAST(REGEXP_REPLACE(population," ","") AS INT) = m
